...
protected:
    ...
    void copyFrom(DLinkedList<T> &list);
...
class BWDIterator
    ...
    public:
        ...
        void remove(void (*removeItemData)(T) = 0)
        {
            pNode->prev->next = pNode->next;
            pNode->next->prev = pNode->prev;
            Node *pNext = pNode->next; // MUST prev, so BWDIterator++ will go to end
            if (removeItemData != 0)
                removeItemData(pNode->data);
            delete pNode;
            pNode = pNext;
...
template <class T>
DLinkedList<T> &DLinkedList<T>::operator=(DLinkedList<T> &list)
{
    //this->removeIntenalData();
    this->copyFrom(list);
    return *this;
}
...
void DLinkedList<T>::add(int index, T e)
{
    ...
    //if (index<count)
    //Node* cur=getPreviousNodeOf(index);
    Node* cur=head;
    for (int i=-1;i<index-1;i++)
        cur=cur->next;
    Node* temp=new Node(e,cur->next,cur);
    cur->next->prev=temp;
    cur->next=temp;
    count++;
}
...
{
    if (index<0||index>=count) throw std::out_of_range("Index is out of range");
    Node* tobedeleted=getPreviousNodeOf(index+1);
    tobedeleted->prev->next=tobedeleted->next;
    tobedeleted->next->prev=tobedeleted->prev;
    tobedeleted->next=NULL;
    tobedeleted->prev=NULL;
    T e=tobedeleted->data;
    this->count--;
    delete tobedeleted;
    return e;
}
...
{
    if (index<0||index>=count) throw std::out_of_range("Index is out of range");
    Node* temp=getPreviousNodeOf(index+1);
    return temp->data;
}
...
int DLinkedList<T>::indexOf(T item)
{
    ...
    while (temp!=tail)
    {
        if (equals(temp->data,item,this->itemEqual)) return i;
        i++;
        temp=temp->next;
    }
    return -1;
...
bool DLinkedList<T>::removeItem(T item, void (*removeItemData)(T))
{
    ...
    {
        if (removeItemData!=NULL) removeItemData(get(indexOf(item)));
...
string DLinkedList<T>::toString(string (*item2str)(T &))
{
    ...
    if (item2str!=NULL) {
        ...
        ss << item2str(get(index)) << ", ";
        if ( count > 0) ss << item2str(get(count-1)) << "]";
        else ss << "]";
    }
    else {
    ss << "[";
    for (int index = 0; index < count - 1; index ++)
    ss << this->get(index) << ", ";
    if ( count > 0) ss << this->get(count - 1) << "]";
    else ss << "]";
    }
    return ss.str();
}
...
template <class T>
void DLinkedList<T>::copyFrom(DLinkedList<T> &list)
{
    /**
