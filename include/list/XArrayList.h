...
public:
    ...
    void setItemEqual(bool (*itemEqual)(T &, T &) = 0)
    {
...
{
    for (int index = 0; index < this->count ; index ++)
    if (this->equals(data[index],item,this->itemEqual))
    return index ;
    return -1;
}
template <class T>
...
string XArrayList<T>::toString(string (*item2str)(T &))
{
    ...
    if (item2str!=NULL) {
        ...
        if ( count > 0) ss << item2str(data[count-1]) << "]";
        else ss << "]";
    }
    else {
    ss << "[";
...
template <class T>
void XArrayList<T>::ensureCapacity(int capacity)
{
    /**
